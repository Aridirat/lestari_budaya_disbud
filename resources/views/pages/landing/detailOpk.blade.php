...
<head>
    ...
    <title>Kebudayaan Takbenda</title>
</head>
<body>
    ...
    <div class="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div class="mt-5 max-w-2xl text-center mx-auto font-merienda">
        <h1 class="block font-bold text-gray-800 text-4xl dark:text-neutral-200">
          <p class="bg-clip-text py-5 bg-linear-to-tl from-cyan-600 via-emerald-400 to-teal-500 md:text-3xl lg:text-5xl text-transparent">Kebudayaan Takbenda</p>
        </h1>
      </div>
      <!-- End Title -->
    </div>
      ...
      <div class="grid grid-cols-9 gap-x-10 gap-y-2 px-3 w-full">
        <div class="col-span-6">
          <div class="space-y-5 md:space-y-8">
              ...
              <div class="space-y-2">
                <h2 class="text-2xl font-bold md:text-5xl dark:text-white">{{ $takbenda->judul_opk }}</h2>
                
                <div class="flex items-center gap-x-5 py-3">
                  <p class="text-xs sm:text-sm text-gray-500 dark:text-neutral-200 mb-1">Berlokasi di <span class="text-gray-800">{{ $takbenda->alamat_tradisi }}</span></p>
                  <div class="block h-3 border-e border-gray-300 mx-3 dark:border-neutral-600"></div>
                  <p class="text-xs sm:text-sm text-gray-500 dark:text-neutral-200 mb-1">Diselenggarakan di <span class="text-gray-800">{{ $takbenda->lokasi_tradisi }}</span></p>
                </div>
              </div>
              
              
              <figure>
                <img class="w-full object-cover rounded-xl" src="{{ asset('storage/' . $takbenda->foto) }}" alt="Blog Image">
                <figcaption class="mt-3 text-sm text-center text-gray-500 dark:text-neutral-500">
                  Foto: {{ $takbenda->judul_opk }}.
                </figcaption>
              </figure>
              <div class="flex items-center gap-x-3">
                <p class="text-xs sm:text-sm text-gray-500 dark:text-neutral-200 mb-1">Narasumber: {{ $takbenda->nama_narasumber }}</p>
                <div class="block h-3 border-e border-gray-300 mx-3 dark:border-neutral-600"></div>
                <p class="text-xs sm:text-sm text-gray-500 dark:text-neutral-200 mb-1">{{ $takbenda->alamat_narasumber }}</p>
              </div>
              
              <p class="text-lg text-gray-800 dark:text-neutral-200">{{ $takbenda->deskripsi }}</p>
              <div>
                  <a class="m-0.5 inline-flex items-center gap-1.5 py-2 px-3 rounded-full text-sm bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-hidden focus:bg-gray-200 dark:bg-neutral-800 dark:text-neutral-200 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700" href="{{ asset('storage/' . $takbenda->dokumen_kajian) }}" target="_blank">
                      Dokumen Kegiatan
                    </a>
              </div>
              <div>
                <div>
                  <p>Video:</p>
                  <iframe class="w-full aspect-video rounded-xl mt-4" src="{{ $takbenda->embedUrl }}" frameborder="0" allowfullscreen></iframe>
              </div>
        ...
        {{-- Side Content --}}
          <div class="col-end-10 col-span-3 lg:w-full lg:h-full lg:bg-linear-to-r lg:from-gray-50 lg:via-transparent lg:to-transparent dark:from-neutral-800">
            <div class="sticky top-0 start-0 py-8 lg:ps-8">
              ...
              <div class="space-y-6">
                <!-- Media -->
                @foreach ($allTakbenda->take(5) as $item)
                    @if ($item->id !== $takbenda->id)
                        <a class="group flex items-center gap-x-6 focus:outline-hidden" href="{{ route('landing.detailOpk', $item->id) }}">
